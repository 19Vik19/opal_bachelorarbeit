...
object Immutability {
  ...
  def evaluate(
    ...
    val classFiles = projectDir match {
      case Some(dir) =>
        JavaClassFileReader().ClassFiles(cp.toPath.resolve(dir).toFile)
    ...
    val allProjectClassTypes =
      project.allProjectClassFiles.iterator.map(_.thisType).toSet

    val allFieldsInProjectClassFiles =
      project.allProjectClassFiles.iterator.flatMap { _.fields }.toSet
    ...
    project.updateProjectInformationKeyInitializationData(AIDomainFactoryKey) {
      _ =>
        if (level == 0)
          ...
          Set[Class[_ <: AnyRef]](
            classOf[domain.l1.DefaultDomainWithCFGAndDefUse[URL]]
          )
        else if (level == 2)
          Set[Class[_ <: AnyRef]](
            classOf[domain.l2.DefaultPerformInvocationsDomainWithCFG[URL]]
          )
        else
            ...
            case Classes =>
              if (css.contains(LazyClassImmutabilityAnalysis))
                allProjectClassTypes.foreach(c =>
                  propertyStore.force(c, ClassImmutability.key)
                )
            case Types =>
              if (css.contains(LazyTypeImmutabilityAnalysis))
                allProjectClassTypes.foreach(c =>
                  propertyStore.force(c, TypeImmutability.key)
                )
            case All =>
              if (css.contains(LazyL2FieldAssignabilityAnalysis))
                allFieldsInProjectClassFiles.foreach(f => {
    ...
    val fieldAssignabilityGroupedResults = propertyStore
      ...
      .filter(field =>
        allFieldsInProjectClassFiles.contains(field.e.asInstanceOf[Field])
      )
      .iterator
      .to(Iterable)
      .groupBy(_.asFinal.p)
    ...
    val fieldGroupedResults = propertyStore
      ...
      .filter(eps =>
        allFieldsInProjectClassFiles.contains(eps.e.asInstanceOf[Field])
      )
      .iterator
      .to(Iterable)
      .groupBy {
        _.asFinal.p match {
          case DependentlyImmutableField(_) =>
            DependentlyImmutableField(SortedSet.empty)
    ...
    val dependentlyImmutableFields =
      fieldGroupedResults // .collect { case (DependentlyImmutableField(_), rhs) => rhs }
    ...
    if (analysis == All || analysis == Fields) {
      stringBuilderResults.append(
        s"""
                ...
                | ${nonTransitivelyImmutableFields
          .map(_ + " | Non Transitively Immutable Field ")
          .mkString("\n")}
                |
          ...
          .mkString("\n")}
                ...
                | ${transitivelyImmutableFields
          .map(_ + " | Transitively Immutable Field ")
          .mkString("\n")}
                |
                |""".stripMargin
      )
    ...
    val classGroupedResults = propertyStore
      ...
      .filter(eps =>
        allProjectClassTypes.contains(eps.e.asInstanceOf[ObjectType])
      )
      .iterator
      .to(Iterable)
      .groupBy {
        _.asFinal.p match {
          case DependentlyImmutableClass(_) =>
            DependentlyImmutableClass(SortedSet.empty)
          case default => default
        }
      }
    ...
    val transitivelyImmutables =
      classGroupedResults.getOrElse(TransitivelyImmutableClass, Iterator.empty)

    val allInterfaces =
      project.allProjectClassFiles
        .filter(_.isInterfaceDeclaration)
        .map(_.thisType)
        .toSet
    ...
    if (analysis == All || analysis == Classes) {
      stringBuilderResults.append(
        s"""
                ...
                | ${nonTransitivelyImmutableClasses
          .map(_ + " | Non Transitively Immutable Class ")
          .mkString("\n")}
                |
          ...
          .mkString("\n")}
                ...
                | ${transitivelyImmutableClasses
          .map(_ + " | Transitively Immutable Classes ")
          .mkString("\n")}
                |
      ...
      .groupBy {
        _.asFinal.p match {
          case DependentlyImmutableType(_) =>
            DependentlyImmutableType(SortedSet.empty)
          case default => default
        }
      }
    ()
    ...
    if (analysis == All || analysis == Types) {
      stringBuilderResults.append(
        s"""
                ...
                | ${nonTransitivelyImmutableTypes
          .map(_ + " | Non-Transitively Immutable Types ")
          .mkString("\n")}
                |
                | Dependently Immutable Types:
                | ${dependentlyImmutableTypes
          .map(_ + " | Dependently Immutable Types ")
          .mkString("\n")}
                |
                | Transitively Immutable Types:
                | ${transitivelyImmutableTypes
          .map(_ + " | Transitively Immutable Types ")
          .mkString("\n")}
                |""".stripMargin
      )
    }
    ...
    if (analysis == Fields || analysis == All) {
      stringBuilderNumber.append(
        s"""
                ...
                | Fields: ${allFieldsInProjectClassFiles.size}
                | Fields with primitive Types / java.lang.String: ${allFieldsInProjectClassFiles
          .filter(field =>
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (i < args.length) {
      args(i) match {
        ...
        case "-withoutConsiderLazyInitialization" =>
          withoutConsiderLazyInitialization = true
