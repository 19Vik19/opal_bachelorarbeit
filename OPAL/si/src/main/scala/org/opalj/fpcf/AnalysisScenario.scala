...
class AnalysisScenario[A](val ps: PropertyStore) {
  ...
  private[this] var collaborativelyDerivedProperties: Set[PropertyBounds] =
    Set.empty
  private[this] var lazilyDerivedProperties: Set[PropertyBounds] = Set.empty

  private[this] var initializationData: Map[ComputationSpecification[A], Any] =
    Map.empty
  ...
  private[this] var batchcounter: Int = 0

  private[this] var eagerCS2: Set[ComputationSpecification[A]] = Set.empty
  private[this] var lazyCS2: Set[ComputationSpecification[A]] = Set.empty
  private[this] var triggeredCS2: Set[ComputationSpecification[A]] = Set.empty
  private[this] var transformersCS2: Set[ComputationSpecification[A]] =
    Set.empty

  private[this] var derivedBy: Map[
    PropertyKind,
    (PropertyBounds, Set[ComputationSpecification[A]])
  ] = {
    Map.empty
  }
  ...
  def +=(cs: ComputationSpecification[A]): this.type = {
    ...
    if (batchcounter == 1) {
      if (cs != null) {
        cs.computationType match {
          case EagerComputation     => eagerCS2 += cs
          case TriggeredComputation => triggeredCS2 += cs
          case LazyComputation      => lazyCS2 += cs
          case Transformer          => transformersCS2 += cs
        }
      }
      else {
          batchcounter += 1
  ...
  def propertyComputationsDependencies: Graph[PropertyBounds] = {
    if (!scheduleComputed) {
      throw new IllegalStateException(
        "initialization incomplete; schedule not computed"
      );
    }
  ...
  def computationDependencies: Graph[ComputationSpecification[A]] = {
    ...
    val derivedBy: Map[PropertyBounds, Set[ComputationSpecification[A]]] = {
      var derivedBy: Map[PropertyBounds, Set[ComputationSpecification[A]]] =
        Map.empty
      allCS foreach { cs =>
  ...
  private[this] def processCS(cs: ComputationSpecification[A]): Unit = {
    ...
    cs.derivesLazily foreach { lazilyDerivedProperty =>
      if (derivedProperties.contains(lazilyDerivedProperty)) {
        ...
        val m =
          s"can not register $cs: $pkName is already computed by another analysis"
    ...
    cs.derivesCollaboratively foreach { collaborativelyDerivedProperty =>
      if (
        eagerlyDerivedProperties.contains(collaborativelyDerivedProperty) ||
    ...
    def handleDerivedProperties(
        derivedProperties: Set[PropertyBounds]
    ): Unit = {
  ...
  def computeSchedule(
      ...
      defaultAnalysis: PropertyBounds => Option[ComputationSpecification[A]] =
        _ => None
  )(implicit
    ...
    val alreadyComputedPropertyKinds =
      propertyStore.alreadyComputedPropertyKindIds.toSet
    ...
    allCS.foreach { cs =>
      cs.derives foreach { derivedProperty =>
        if (alreadyComputedPropertyKinds.contains(derivedProperty.pk.id)) {
          ...
          val m =
            s"can not register $cs: $pkName was computed in a previous phase"
          throw new SpecificationViolation(m)
        }
      }
    }
    ...
    def useFallback(underivedProperty: PropertyBounds, propertyName: String) = {
      if (PropertyKey.hasFallback(underivedProperty.pk)) {
        val message =
          s"no analyses scheduled for: $propertyName; using fallback"
        OPALLogger.warn("analysis configuration", message)
      } else {
        throw new IllegalStateException(
          s"no analysis scheduled for $propertyName"
        )
      }
    }
        ...
        if (!derivedProperties.contains(underivedProperty)) {
          ...
          if (defaultCSOpt.isDefined) {
            ...
            try {
              processCS(defaultCS)
              val message =
                s"no analyses scheduled for: $propertyName; using ${defaultCS.name}"
    ...
    OPALLogger.info("Schedular", "Starting Schedule")
      // TODO ...
      var computedPhases: List[PhaseConfiguration[A]] = List.empty
      ...
      val returnschedule = Schedule(
          if (allCS.isEmpty) {
              ...
              val phases = (0 to 1).toList.map(i => computePhase(i))
              computedPhases = phases
              phases
          },
          initializationData
          )

    OPALLogger.info("Schedular", "Schedule completed")
  ...
  /** Computes the configuration for a specific batch; this method can only handle the situation
    ...
    */
  private def computePhase(counter: Int): PhaseConfiguration[A] = {
    ...
    transformersCS foreach { cs =>
      suppressInterimUpdates += (cs.derivesLazily.get.pk -> cs
        .uses(ps)
        .map(_.pk))
    }
    ...
    if (counter == 0){
        ...
        batchBuilder ++= lazyCS2
        batchBuilder ++= transformersCS2
        batchBuilder ++= triggeredCS2
        batchBuilder ++= eagerCS2
    }
    // FIXME...
...
object AnalysisScenario {
  ...
  final val AnalysisAutoConfigKey =
    "org.opalj.fpcf.AnalysisScenario.AnalysisAutoConfig"
